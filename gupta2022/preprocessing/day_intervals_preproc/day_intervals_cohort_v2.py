...
def get_visit_pts(mimic4_path:str, group_col:str, visit_col:str, admit_col:str, disch_col:str, adm_visit_col:str, use_mort:bool, use_los:bool, los:int, use_admn:bool, disease_label:str,use_ICU:bool):
    """
    Apparently 'visit' is the moment the patient enter in the hospital ou something like that.
    ----
    ...
    use_ICU: describes whether to specifically look at ICU visits in icu/icustays OR look at general admissions from core/admissions
    """
...
# * ############ Working on
def extract_data(use_ICU:str, label:str, time:int, icd_codes:list | str, root_dir, data_dir, disease_label, cohort_output=None, summary_output=None):
    ...
    if icd_codes=="No Disease Filter":
        if len(disease_label):
        ...
        if len(disease_label):
            print(f"EXTRACTING FOR: | {use_ICU.upper()} | {label.upper()} DUE TO {disease_label.upper()} | ADMITTED DUE TO {(icd_code.upper() for icd_code in icd_codes)} | {str(time)} |")
        else:
            print(f"EXTRACTING FOR: | ICU {use_ICU.upper()} | LABEL/TASK {label.upper()} | ADMITTED DUE TO {(icd_code.upper() for icd_code in icd_codes)} | TIME {str(time)} |")
    #print(label)
    ...
    # * ICU OR HOSP - Use ICU (Intensive Care Unit patients)
    use_ICU = use_ICU == "ICU" # change to boolean value
    
    # * Use cohort?
    use_disease=icd_codes!="No Disease Filter"
    ...
    pts.to_csv("./data/test/visits.csv")
