...
def feature_nonicu(cohort_output, data_path, version_path, diag_flag=True,lab_flag=True,proc_flag=True,med_flag=True):
    if diag_flag:
        print("[EXTRACTING DIAGNOSIS DATA]")
        diag = preproc_icd_module(os.path.join(data_path, version_path)+"/hosp/diagnoses_icd.csv.gz", './data/cohort/'+cohort_output+'.csv.gz', './utils/mappings/ICD9_to_ICD10_mapping.txt', map_code_colname='diagnosis_code')
...
#         out[['subject_id', 'hadm_id', 'stay_id', 'itemid', 'charttime', 'intime', 'event_time_from_admit']].to_csv("./data/features/preproc_out_icu.csv.gz", compression='gzip', index=False)
    ...
    if proc_flag:
        ...
        proc = preproc_proc(os.path.join(data_path, version_path)+"/hosp/procedures_icd.csv.gz",'./data/cohort/'+cohort_output+'.csv.gz', 'chartdate', 'base_anchor_year', dtypes=None, usecols=None)
    ...
    if med_flag:
        ...
        med = preproc_meds(os.path.join(data_path, version_path)+"/hosp/prescriptions.csv.gz", './data/cohort/'+cohort_output+'.csv.gz','./utils/mappings/ndc_product.txt')
    ...
    if lab_flag:
        ...
        # print(data_path, version_path)
        lab = preproc_labs(os.path.join(data_path, version_path)+"/hosp/labevents.csv.gz", data_path, version_path,'./data/cohort/'+cohort_output+'.csv.gz','charttime', 'base_anchor_year', dtypes=None, usecols=None)
